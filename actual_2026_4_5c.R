...
library("missForest")#to impute NAs, preparation for random forest
...
#Begin preference versus HH income figure####
...
#End preference versus HH income figure####

#+++++++++++++++

#Begin age versus PI preference figure####
...
# Construct and income groups
i2_inperson5.df =i2_inperson4.df %>%
  mutate(
    age_group = cut(
      age,
      breaks = c(17, 24, 34, 44, 54, 64, 75, Inf),
...
# number who prefer cash by age category
...
#Start random forest variable importance table####
r1.df = m9.df
# pi
class(r1.df$pi)
...
r1.df = r1.df %>% mutate(accept_card = ifelse(accept_card==3, NA, accept_card))# make 3 (don't know) as NA
table(r1.df$accept_card, useNA = "always")
...
r1.df = r1.df %>% mutate(accept_cash = ifelse(accept_cash==5, NA, accept_cash))# make 5 (don't know) as NA
table(r1.df$accept_cash, useNA = "always")
r1.df = r1.df %>% mutate(accept_cash = ifelse(accept_cash==2, 0, accept_cash))# make 2 a 0 (no)
table(r1.df$accept_cash, useNA = "always")
r1.df = r1.df %>% mutate(accept_cash = ifelse(accept_cash==3, 1, accept_cash))# make 3 a 1 (yes)
table(r1.df$accept_cash, useNA = "always")
r1.df = r1.df %>% mutate(accept_cash = ifelse(accept_cash==4, 0, accept_cash))# make 4 a 0 (no)
table(r1.df$accept_cash, useNA = "always")
...
set.seed(1955)
sum(is.na(r4.df))
r4_imp_start_time =Sys.time()
r4_imp = missForest(r4.df, backend = "randomForest")
r4_imp_end_time =Sys.time()
r4_imp_end_time -r4_imp_start_time# duration of imputing
str(r4_imp)
r4_imputed.df =r4_imp$ximp
names(r4_imputed.df)
sum(is.na(r4_imputed.df))# verify no NAs in imputed data

# Compare some ratios of the imputed data to the un-imputed data to verify only small changes
(frac_accept_card_imputed = table(r4_imputed.df$accept_card)[2]/(table(r4_imputed.df$accept_card)[1]+table(r4_imputed.df$accept_card)[2]))
frac_accept_card
#
(frac_accept_cash_imputed = table(r4_imputed.df$accept_cash)[2]/(table(r4_imputed.df$accept_cash)[1]+table(r4_imputed.df$accept_cash)[2]))
frac_accept_cash
#
(frac_cc_surcharge_imputed = table(r4_imputed.df$cc_surcharge)[2]/(table(r4_imputed.df$cc_surcharge)[1]+table(r4_imputed.df$cc_surcharge)[2]))
frac_cc_surcharge
#
(frac_discount_imputed = table(r4_imputed.df$discount)[2]/(table(r4_imputed.df$discount)[1]+table(r4_imputed.df$discount)[2]))
frac_discount
...
#End random forest variable importance table####

#Begin random forest confusion matrix####
...
str(r4.df)
sapply(r4.df, class)
# imputing NAs for training data
r4_train_imp_start_time =Sys.time()
r4_train_imp = missForest(r4_train.df, backend = "randomForest")
r4_train_imp_end_time =Sys.time()
r4_train_imp_end_time -r4_train_imp_start_time
str(r4_train_imp)
r4_train_imputed.df =as.data.frame(r4_train_imp$ximp)
dim(r4_train_imputed.df)
sum(is.na(r4_train_imputed.df))# verify no NAs anymore

# imputing NAs for testing data
r4_test_imp_start_time =Sys.time()
r4_test_imp = missForest(r4_test.df, backend = "randomForest")
r4_test_imp_end_time =Sys.time()
r4_test_imp_end_time -r4_test_imp_start_time
str(r4_test_imp)
r4_test_imputed.df =as.data.frame(r4_test_imp$ximp)
dim(r4_test_imputed.df)
dim(r4_train_imputed.df) +dim(r4_test_imputed.df)#=> num payments
sum(is.na(r4_test_imputed.df))# verify no NAs anymore
...
#End random forest confusion matrix####
...
#Begin: Classification trees figure####
names(r4_imputed.df)
...
(tree_model1 = pi ~ Preference + Merchant + Amount + accept_card + accept_cash +  cc_surcharge + discount)
...
#End: Classification trees figure####
