...
# This code uses the 2025 translation-level and individual public data
...
setwd("~/SDCPC/2025_SDCPC")# work
#setwd("~/Research/SCPC_DCPC/2025_SDCPC")# home
dir()
...
i2.df = subset(indiv2025_1.df, select = c(id, paypref_inperson, paypref_b1, paypref_web, hhincome, income_hh, age, cc_adopt, chk_adopt, chk_adopt, computer_adopt, csh_adopt, crypto_adopt, dc_adopt, elect_adopt, mb_adopt, mobile_adopt, mobile_inperson_adopt, cc_rewards))
...
# Check card adoption: cc_adopt and dc_adopt => see Misc section at the end
# cc adoption
table(i2.df$cc_adopt, useNA = "always")
round(prop.table(table(i2.df$cc_adopt)), 2)
# db adoption
table(i2.df$dc_adopt, useNA = "always")
round(prop.table(table(i2.df$dc_adopt)),2)
# Are there resp with bank accounts that don't have a dc?

# Construct a data frame with preferred PI for in-person payments
...
# exclude cc_rewards b/c of NAs (to be fixed for random forest)
#sum(is.na(i2_inperson1.df$paypref_inperson))
table(i2_inperson1.df$paypref_inperson, useNA = "always")

# Group preferences for PI into 4 preferred payment methods (instead of 13 payment methods)
...
# check for missing values & deleting them (except for cc_reward that has many NA, to be taken care off in random forest)
sum(is.na(i2_inperson2.df))
i2_inperson3.df = i2_inperson2.df[complete.cases(i2_inperson2.df), ]
dim(i2_inperson2.df)# before
dim(i2_inperson3.df)# after
...
# checksum columns
sum(pref_inperson1.df)-sum(1:16)# should equal number of respondents
...
# Info about Figure 2 (preferences vs. age) (in the paper, new subsection)
sum(num_resp_by_age.df$n)# equal number of resp
# number of resp with age NA deleted
pref_inperson_age3.df[8, "num_resp" ]
# Hence, respondents in Figure preference frac vs. age
...
#Begin: Table 1: Preferences versus card ownership and rewards####
versus1.df = subset(i2.df, select = c("paypref_inperson", "cc_adopt", "dc_adopt", "cc_rewards"))
names(versus1.df)
dim(versus1.df)
str(versus1.df)
#
versus2.df = versus1.df
table(versus1.df$paypref_inperson, useNA = "always")
sum(table(versus2.df$paypref_inperson))# num resp
#
# Group preferences into 4 PI only
versus2.df = versus2.df %>%
  mutate(paypref_inperson = case_when(
    paypref_inperson == 1 ~ "cash",
    paypref_inperson == 3 ~ "credit",
    paypref_inperson == 4 ~ "debit",
...
sample_n(i2_inperson2.df,20)
dim(i2_inperson2.df)
versus2.df$paypref_inperson = as.factor(versus2.df$paypref_inperson)
table(versus2.df$paypref_inperson, useNA = "always")
#delete paypref with NA
dim(versus2.df)
versus3.df = versus2.df[!is.na(versus2.df$paypref_inperson) & !is.na(versus2.df$cc_adopt) & !is.na(versus2.df$dc_adopt), ]
dim(versus3.df)
#
# cc_adopt adjustments
class(versus3.df$cc_adopt)
table(versus3.df$cc_adopt, useNA = "always")
versus3.df$cc_adopt =as.factor(versus3.df$cc_adopt)
levels(versus3.df$cc_adopt)
levels(versus3.df$cc_adopt)[levels(versus3.df$cc_adopt)==0] ="No"
levels(versus3.df$cc_adopt)[levels(versus3.df$cc_adopt)==1] ="Yes"
table(versus3.df$cc_adopt, useNA = "always")
#
# dc_adopt adjustments
class(versus3.df$dc_adopt)
table(versus3.df$dc_adopt, useNA = "always")
versus3.df$dc_adopt =as.factor(versus3.df$dc_adopt)
levels(versus3.df$dc_adopt)
levels(versus3.df$dc_adopt)[levels(versus3.df$dc_adopt)==0] ="No"
levels(versus3.df$dc_adopt)[levels(versus3.df$dc_adopt)==1] ="Yes"
table(versus3.df$dc_adopt, useNA = "always")
#
# cc_rewards adjustments
class(versus3.df$cc_rewards)
table(versus3.df$cc_rewards, useNA = "always")
versus3.df$cc_rewards =as.factor(versus3.df$cc_rewards)
levels(versus3.df$cc_rewards)
levels(versus3.df$cc_rewards)[levels(versus3.df$cc_rewards)==0] ="No"
levels(versus3.df$cc_rewards)[levels(versus3.df$cc_rewards)==1] ="Yes"
table(versus3.df$cc_rewards, useNA = "always")
# How many reward NAs of credit owners?
table(versus3.df[versus3.df$cc_adopt=="Yes", ]$cc_rewards, useNA = "always")
# delete this NA
versus3.df %>% filter(cc_adopt=="Yes" & is.na(cc_rewards))
(reward_na = which(versus3.df$cc_adopt=="Yes" & is.na(versus3.df$cc_rewards)))
v4.df = versus3.df[-reward_na ,]# renaming versus to v
dim(versus3.df)
dim(v4.df)

#start constructing a table crossing the above with preferences
#
# num prefer credit
(num_ccprefer = nrow(v4.df[v4.df$paypref_inperson=="credit", ]))
# num resp who prefer credit and own credit
(num_ccprefer_ccown = nrow(v4.df[v4.df$paypref_inperson=="credit" & v4.df$cc_adopt=="Yes", ]))
# num resp who prefer credit and not own credit
(num_ccprefer_ccnotown = nrow(v4.df[v4.df$paypref_inperson=="credit" & v4.df$cc_adopt=="No", ]))
# num resp who prefer credit and own credit and reward credit
(num_ccprefer_ccown_reward = nrow(v4.df[v4.df$paypref_inperson=="credit" & v4.df$cc_adopt=="Yes" & v4.df$cc_rewards=="Yes", ]))
# num resp who prefer credit and own credit and no reward credit
(num_ccprefer_ccown_noreward = nrow(v4.df[v4.df$paypref_inperson=="credit" & v4.df$cc_adopt=="Yes" & v4.df$cc_rewards=="No", ]))
#
# num not prefer credit
(num_ccnotpref = nrow(v4.df[v4.df$paypref_inperson!="credit", ]))
# num resp who don't prefer credit and own credit
(num_ccnotprefer_ccown = nrow(v4.df[v4.df$paypref_inperson!="credit" & v4.df$cc_adopt=="Yes", ]))
# num resp who don't prefer credit and not own credit
(num_ccnotprefer_ccnotown = nrow(v4.df[v4.df$paypref_inperson!="credit" & v4.df$cc_adopt=="No", ]))
# num resp who don't prefer credit and own credit and reward credit
(num_ccnotprefer_ccown_reward = nrow(v4.df[v4.df$paypref_inperson!="credit" & v4.df$cc_adopt=="Yes" & v4.df$cc_rewards=="Yes", ]))
# num resp who don't prefer credit and own credit and no reward credit
(num_ccnotprefer_ccown_notreward = nrow(v4.df[v4.df$paypref_inperson!="credit" & v4.df$cc_adopt=="Yes" & v4.df$cc_rewards=="No", ]))
#
# num prefer debit
(num_dcprefer = nrow(v4.df[v4.df$paypref_inperson=="debit", ]))
# num resp who prefer debit and own debit
(num_dcprefer_dcown = nrow(v4.df[v4.df$paypref_inperson=="debit" & v4.df$dc_adopt=="Yes", ]))
# num resp who prefer debit and not own debit
(num_dcprefer_dcnotown = nrow(v4.df[v4.df$paypref_inperson=="debit" & v4.df$dc_adopt=="No", ]))

# num don't prefer debit
(num_dcnotprefer = nrow(v4.df[v4.df$paypref_inperson!="debit", ]))
# num resp who don't prefer debit and own debit
(num_dcnotprefer_dcown = nrow(v4.df[v4.df$paypref_inperson!="debit" & v4.df$dc_adopt=="Yes", ]))
# num resp who don't prefer debit and not own debit
(num_dcnotprefer_dcnotown = nrow(v4.df[v4.df$paypref_inperson!="debit" & v4.df$dc_adopt=="No", ]))

#construct columns from the above
(ccprefer.vec = c(num_ccprefer, num_ccprefer_ccown, num_ccprefer_ccnotown, num_ccprefer_ccown_reward, num_ccprefer_ccown_noreward))
#
(ccnotpref.vec = c(num_ccnotpref, num_ccnotprefer_ccown, num_ccnotprefer_ccnotown, num_ccnotprefer_ccown_reward, num_ccnotprefer_ccown_notreward))
#

(dcprefer.vec = c(num_dcprefer, num_dcprefer_dcown, num_dcprefer_dcnotown, NA, NA))
#
(dcnotprefer.vec = c(num_dcnotprefer, num_dcnotprefer_dcown, num_dcnotprefer_dcnotown, NA, NA))

# make it a data frame
(ownership.vec = c("Total", "Own card", "Do not own card", "Own card with reward", "Own_card without reward"))
#
(v.df = data.frame(ownership.vec, ccprefer.vec, ccnotpref.vec, dcprefer.vec, dcnotprefer.vec))

print(xtable(v.df), hline.after =c(0,1,3,5), include.rownames=F)

# Information provided below Table 1 (Findings 1)
nrow(v4.df)# num resp
names(v.df)
#
#Findings 1a
# num credit card owners
(num_cc_own = num_ccprefer_ccown+num_ccnotprefer_ccown)
# prefer and own CC
num_ccprefer_ccown
# frac who own credit and prefer credit
round(num_ccprefer_ccown/num_cc_own, 2)
# own CC but do NOT prefer using it
num_ccnotprefer_ccown
# frac who own credit and don't prefer credit
round(num_ccnotprefer_ccown/num_cc_own, 2)

#Findings 1b
# num debit card owners
(num_dc_own = num_dcprefer_dcown+num_dcnotprefer_dcown)
# num dc owners who prefer dc
num_dcprefer_dcown
#fraction dc prefer out of DC owners
round(num_dcprefer_dcown/num_dc_own,2)
# num dc owners who do not prefer
num_dcnotprefer_dcown
# frac dc owners who do not prefer
round(num_dcnotprefer_dcown/num_dc_own,2)

#Findings 1c
#num who prefer CC
num_ccprefer
#num prefer and own cc
num_ccprefer_ccown
# frac prefer and own cc
round(num_ccprefer_ccown/num_ccprefer,2)
# num prefer cc but do not own cc
num_ccprefer_ccnotown
# as frac
round(num_ccprefer_ccnotown/num_ccprefer,2)

#Findings 1d
#num who prefer dC
num_dcprefer
#num prefer and own dc
num_dcprefer_dcown
# frac prefer and own dc
round(num_dcprefer_dcown/num_dcprefer,2)
# num prefer dc but do not own dc
num_dcprefer_dcnotown
# as frac
round(num_dcprefer_dcnotown/num_dcprefer,2)

#Findings 1e
#num own and prefer CC
num_ccprefer_ccown
# num prefer and own CC wit rewards
num_ccprefer_ccown_reward
# as frac
round(num_ccprefer_ccown_reward/num_ccprefer_ccown,2)
# num who own CC but do not prefer CC
num_ccnotprefer_ccown
# num own non reward CC and do not prefer
num_ccnotprefer_ccown_reward
# as frac
round(num_ccnotprefer_ccown_reward/num_ccnotprefer_ccown,2)
# End of Findings 1

#End: Table 1: Preferences versus card ownership and rewards####

#+++++++++++++++

#Begin: Table 2: Confusion matrix####
#rows: Actually used PI (reference)
#columns: Preferred payment method (predictor = data)

# merge the two original datasets by id 
m1.df = left_join(trans2025_1.df, indiv2025_1.df, by = "id")
dim(m1.df)
names(m1.df)
...
m3.df = subset(m2.df, select = c(id, ind_weight_all, cc_adopt, dc_adopt, cc_rewards, pi, merch, gender, age, amnt, paypref_inperson, accept_cash, accept_card, cc_surcharge, discount))
...
(m8.df = m7.df %>% group_by(merch) %>% filter(n() >= 250))
...
#Definition 1 (TP examples)
tp_cash
round(tp_cash/(confusion3.df["Cash", "Total"]),2)
#
tp_credit
round(tp_credit/(confusion3.df["Credit", "Total"]),2)
#
tp_debit
round(tp_debit/(confusion3.df["Debit", "Total"]),2)

#Definition 2 (FN examples)
fn_cash
(confusion3.df["Credit", "Cash"] + confusion3.df["Debit", "Cash"] + confusion3.df["Other", "Cash"])
1168+1193+173
#
fn_credit
(confusion3.df["Cash", "Credit"] + confusion3.df["Debit", "Credit"] + confusion3.df["Other", "Credit"])
291+614+186
#
fn_debit
(confusion3.df["Cash", "Debit"] + confusion3.df["Credit", "Debit"] + confusion3.df["Other", "Debit"])
465+437+219

#Definition 3 (False positive examples)
fp_cash
(confusion3.df["Cash", "Debit"] + confusion3.df["Cash", "Credit"] + confusion3.df["Cash", "Other"])
291+465+141
#
fp_credit
(confusion3.df["Credit", "Debit"] + confusion3.df["Credit", "Cash"] + confusion3.df["Credit", "Other"])
1168+437+567

#Definition 4 (TN examples)
(total_in_table = sum(confusion3.df$Total))
tn_cash
total_in_table - tp_cash -fn_cash - fp_cash 
#
tn_credit
total_in_table - tp_credit -fn_credit - fp_credit 
#
tn_debit
total_in_table - tp_debit -fn_debit - fp_debit 
#

# Bottom part of Table 2
(sen_cash = tp_cash/(tp_cash + fn_cash))
round(sen_cash,2)
#
(spe_cash = tn_cash/(tn_cash + fp_cash))
round(spe_cash,2)
#
(ppv_cash = tp_cash/(tp_cash + fp_cash))
round(ppv_cash,2)
#
(npv_cash = tn_cash/(tn_cash + fn_cash))
round(npv_cash,2)
#
(ba_cash = (sen_cash + spe_cash)/2)
round(ba_cash,2)
#
#
(sen_credit = tp_credit/(tp_credit + fn_credit))
round(sen_credit,2)
#
(spe_credit = tn_credit/(tn_credit + fp_credit))
round(spe_credit,2)
#
(ppv_credit = tp_credit/(tp_credit + fp_credit))
round(ppv_credit,2)
#
(npv_credit = tn_credit/(tn_credit + fn_credit))
round(npv_credit,2)
#
(ba_credit = (sen_credit + spe_credit)/2)
round(ba_credit,2)
#
#
(sen_debit = tp_debit/(tp_debit + fn_debit))
round(sen_debit,2)
#
(spe_debit = tn_debit/(tn_debit + fp_debit))
round(spe_debit,2)
#
(ppv_debit = tp_debit/(tp_debit + fp_debit))
round(ppv_debit,2)
#
(npv_debit = tn_debit/(tn_debit + fn_debit))
round(npv_debit,2)
#
(ba_debit = (sen_debit + spe_debit)/2)
round(ba_debit,2)
#
#
(sen_other = tp_other/(tp_other + fn_other))
round(sen_other,2)
#
(spe_other = tn_other/(tn_other + fp_other))
round(spe_other,2)
#
(ppv_other = tp_other/(tp_other + fp_other))
round(ppv_other,2)
#
(npv_other = tn_other/(tn_other + fn_other))
round(npv_other,2)
#
(ba_other = (sen_other + spe_other)/2)
round(ba_other,2)

#End: Table 2: Confusion matrix####

#+++++++++++++++++++++

#Begin: Table 3 random forest variable importance####
r1.df = m9.df
names(r1.df)
# pi
class(r1.df$pi)
...
table(r1.df$cc_adopt, useNA = "always")
# rename cc_adopt 
r1.df = r1.df %>% rename(credit_own = cc_adopt)
names(r1.df)
#
## dc_adopt 
...
table(r1.df$dc_adopt, useNA = "always")
# rename dc_adopt 
r1.df = r1.df %>% rename(debit_own = dc_adopt)
names(r1.df)
#
## cc rewards
class(r1.df$cc_rewards)
table(r1.df$cc_rewards, useNA = "always")
r1.df$cc_rewards =as.factor(r1.df$cc_rewards)
levels(r1.df$cc_rewards)
levels(r1.df$cc_rewards)[levels(r1.df$cc_rewards)==0] ="No"
levels(r1.df$cc_rewards)[levels(r1.df$cc_rewards)==1] ="Yes"
table(r1.df$cc_rewards, useNA = "always")
# rename cc_reward
r1.df = r1.df %>% rename(credit_reward = cc_rewards)
names(r1.df)
#
## accept_card
...
# shift all credit NA to 1 (Yes) because the payment was made with a credit card (hence, CC was accepted)
r1.df = r1.df %>% mutate(accept_card = ifelse(is.na(accept_card) & pi =="Credit", 1, accept_card))
table(r1.df$accept_card, useNA = "always")
r1.df %>% group_by(pi) %>% summarise(count = sum(is.na(accept_card)))
# shift all debit NA to 1 (Yes) because the payment was made with a DC, hence it was accepted.
...
# shift all cash NA to 1 (Yes) because the payment was made with cash
...
# remove unneeded vars (remove gender and age)
names(r3.df)
r4.df = subset(r3.df, select = c(id, ind_weight_all, pi, merch, amnt, credit_own, debit_own, credit_reward, paypref_inperson, accept_cash, accept_card, cc_surcharge, discount))
...
nrow(r4.df[is.na(r4.df$credit_own), ])#don't impute
nrow(r4.df[is.na(r4.df$debit_own), ])#don't impute
nrow(r4.df[is.na(r4.df$credit_reward), ])#don't impute because it applies to credit cards only 
...
#Set all credit_reward==No for non-credit card payments. The reason is that random forest cannot handle NA. Although  classification trees can handle NA, I also use this modification  for the classification tree anyway.
r5_imputed.df = r4_imputed.df
class(r5_imputed.df$credit_reward)
r5_imputed.df$credit_reward =as.factor(r5_imputed.df$credit_reward)
#
table(r5_imputed.df$pi, useNA = "always")
table(r5_imputed.df$credit_reward, useNA = "always")
# change credit_reward NA to No for non-CC payments made by non-cC owners 
r5_imputed.df = r5_imputed.df %>% mutate(credit_reward = if_else(is.na(credit_reward) & pi !="Credit" & credit_own =="No", "No", credit_reward))
table(r5_imputed.df$pi, useNA = "always")
table(r5_imputed.df$credit_reward, useNA = "always")
# change NA to No for payments made by credit card non-owners
r5_imputed.df = r5_imputed.df %>% mutate(credit_reward = if_else(is.na(credit_reward) & credit_own =="No", "No", credit_reward))
r5_imputed.df[is.na(r5_imputed.df$credit_reward), ]
# change CC reward NA to Yes, for the remaining NA
r5_imputed.df = r5_imputed.df %>% mutate(credit_reward = if_else(is.na(credit_reward), "No", credit_reward))
table(r5_imputed.df$credit_reward, useNA = "always")
r5_imputed.df$credit_reward =as.factor(r5_imputed.df$credit_reward)
...
(frac_accept_card_imputed = table(r5_imputed.df$accept_card)/nrow(r5_imputed.df))
frac_accept_card
#
(frac_accept_cash_imputed = table(r5_imputed.df$accept_cash)/nrow(r5_imputed.df))
frac_accept_cash
#
(frac_cc_surcharge_imputed = table(r5_imputed.df$cc_surcharge)/nrow(r5_imputed.df))
frac_cc_surcharge
#
(frac_discount_imputed = table(r5_imputed.df$discount)/nrow(r5_imputed.df))
frac_discount
#
str(r5_imputed.df)

# Model
#forest_model1 = pi ~ paypref_inperson + merch + amnt + accept_card + accept_cash + cc_surcharge + discount #without credit_own, debit_own
forest_model1 = pi ~ paypref_inperson + merch + amnt + credit_own + credit_reward + debit_own + accept_card + accept_cash + cc_surcharge + discount

# random forest
set.seed(1955)
(forest_output =randomForest(forest_model1, data=r5_imputed.df, importance=T, na.action=na.roughfix))
...
# after imputation (used)
nrow(r5_imputed.df)# num of payments
nrow(r5_imputed.df[unique(r5_imputed.df$id), ])# num respondents


#End: Table 3: random forest variable importance####

#+++++++++++++++

#Begin: Table 4 random forest confusion matrix####
...
# need to fix the card reward as I have done before, but now, before the training
r5.df = r4.df
#
class(r5.df$credit_reward)
r5.df$credit_reward =as.factor(r5.df$credit_reward)
#
table(r5.df$pi, useNA = "always")
table(r5.df$credit_reward, useNA = "always")
# change NA to No for payments non-credit card payments nade by non-CC owners
r5.df = r5.df %>% mutate(credit_reward = if_else(is.na(credit_reward) & pi !="Credit" & credit_own =="No", "No", credit_reward))
table(r5.df$pi, useNA = "always")
table(r5.df$credit_reward, useNA = "always")
# change NA to No for payments credit non-owners
r5.df = r5.df %>% mutate(credit_reward = if_else(is.na(credit_reward) & credit_own =="No", "No", credit_reward))
r5.df[is.na(r5.df$credit_reward), ]
# change NA to Yes, for the remaining NA
r5.df = r5.df %>% mutate(credit_reward = if_else(is.na(credit_reward), "No", credit_reward))
table(r5.df$credit_reward, useNA = "always")
r5.df$credit_reward =as.factor(r5.df$credit_reward)

set.seed(1955)# may not be needed if the previous one is still in effect
r5_train_index = sample(nrow(r5.df), 0.7*nrow(r5.df))
length(r5_train_index)
#
r5_train.df = r5.df[r5_train_index, ]
r5_test.df = r5.df[-r5_train_index, ]
dim(r5_train.df)
dim(r5_test.df)
dim(r5_train.df) + dim(r5_test.df)# must equal nrow r5.df

#Both the training and testing data must be imputed for NAs
str(r5.df)
sapply(r5.df, class)
# impute the training data
r5_imp_accept_cash_train = missRanger(data = r5_train.df, formula =  accept_cash ~ pi + amnt + merch)
#
r5_imp_accept_card_train = missRanger(data = r5_train.df, formula = pi + accept_card ~ pi + amnt + merch)
#
r5_imp_cc_surcharge_train = missRanger(data = r5_train.df, formula = cc_surcharge ~ pi + amnt + merch)
#
r5_imp_discount_train = missRanger(data = r5_train.df, formula = discount ~ pi + amnt + merch)
#
# Merge the 4 training imputations into a single data frame (in sequence)
names(r5_imp_accept_cash_train)
r5_train_imputed.df = r5_imp_accept_cash_train
r5_train_imputed.df$accept_card = r5_imp_accept_card_train$accept_card
r5_train_imputed.df$cc_surcharge = r5_imp_cc_surcharge_train$cc_surcharge
r5_train_imputed.df$discount = r5_imp_accept_card_train$discount
#
head(r5_train_imputed.df)
sum(is.na(r5_train_imputed.df))# remaining NA?
nrow(r5_train_imputed.df)

# impute the test data
r5_imp_accept_cash_test = missRanger(data = r5_test.df, formula = accept_cash ~ pi + amnt + merch)
#
r5_imp_accept_card_test = missRanger(data = r5_test.df, formula = accept_card ~ pi + amnt + merch)
#
r5_imp_cc_surcharge_test = missRanger(data = r5_test.df, formula = cc_surcharge ~ pi + amnt + merch)
#
r5_imp_discount_test = missRanger(data = r5_test.df, formula = discount ~ pi + amnt + merch)
#
# Merge the 4 test imputations into a single data frame (in sequence)
names(r5_imp_accept_cash_test)
r5_test_imputed.df = r5_imp_accept_cash_test
r5_test_imputed.df$accept_card = r5_imp_accept_card_test$accept_card
r5_test_imputed.df$cc_surcharge = r5_imp_cc_surcharge_test$cc_surcharge
r5_test_imputed.df$discount = r5_imp_discount_test$discount
#
head(r5_test_imputed.df)
sum(is.na(r5_test_imputed.df))# remaining NA?
nrow(r5_test_imputed.df)

dim(r5_train_imputed.df) +dim(r5_test_imputed.df)#=> num payments
sum(is.na(r5_test_imputed.df))# NA remained?

#Now, use only training data to prepare prediction
forest_model1
(forest_output_train=randomForest(forest_model1, data=r5_train_imputed.df, importance=T, na.action=na.roughfix)) # impute NAs
...
sum(is.na(r5_test_imputed.df))
dim(r5_test_imputed.df)

# Predict using the test data
forest_pred=predict(forest_output_train, newdata = r5_test_imputed.df)# predict test data
...
(forest_confusion.t = confusionMatrix(forest_pred, r5_test_imputed.df$pi))
# convert it to dataframe
...
nrow(r5.df)# number of payments
nrow(r5.df[unique(r5.df$id), ])# num respondents
#
nrow(r5_train_imputed.df)# num payments in the training subsample
nrow(r5_test_imputed.df)# num payments in the testing subsample

# Bottom part of Table 4
(forest_sen_cash = forest_tp_cash/(forest_tp_cash + forest_fn_cash))
round(forest_sen_cash,2)
#
(forest_spe_cash = forest_tn_cash/(forest_tn_cash + forest_fp_cash))
round(forest_spe_cash,2)
#
(forest_ppv_cash = forest_tp_cash/(forest_tp_cash + forest_fp_cash))
round(forest_ppv_cash,2)
#
(forest_npv_cash = forest_tn_cash/(forest_tn_cash + forest_fn_cash))
round(forest_npv_cash,2)
#
(forest_ba_cash = (forest_sen_cash + forest_spe_cash)/2)
round(forest_ba_cash,2)
#
#
(forest_sen_credit = forest_tp_credit/(forest_tp_credit + forest_fn_credit))
round(forest_sen_credit,2)
#
(forest_spe_credit = forest_tn_credit/(forest_tn_credit + forest_fp_credit))
round(forest_spe_credit,2)
#
(forest_ppv_credit = forest_tp_credit/(forest_tp_credit + forest_fp_credit))
round(forest_ppv_credit,2)
#
(forest_npv_credit = forest_tn_credit/(forest_tn_credit + forest_fn_credit))
round(forest_npv_credit,2)
#
(forest_ba_credit = (forest_sen_credit + forest_spe_credit)/2)
round(forest_ba_credit,2)
#
#
(forest_sen_debit = forest_tp_debit/(forest_tp_debit + forest_fn_debit))
round(forest_sen_debit,2)
#
(forest_spe_debit = forest_tn_debit/(forest_tn_debit + forest_fp_debit))
round(forest_spe_debit,2)
#
(forest_ppv_debit = forest_tp_debit/(forest_tp_debit + forest_fp_debit))
round(forest_ppv_debit,2)
#
(forest_npv_debit = forest_tn_debit/(forest_tn_debit + forest_fn_debit))
round(forest_npv_debit,2)
#
(forest_ba_debit = (forest_sen_debit + forest_spe_debit)/2)
round(forest_ba_debit,2)
#
#
(forest_sen_other = forest_tp_other/(forest_tp_other + forest_fn_other))
round(forest_sen_other,2)
#
(forest_spe_other = forest_tn_other/(forest_tn_other + forest_fp_other))
round(forest_spe_other,2)
#
(forest_ppv_other = forest_tp_other/(forest_tp_other + forest_fp_other))
round(forest_ppv_other,2)
#
(forest_npv_other = forest_tn_other/(forest_tn_other + forest_fn_other))
round(forest_npv_other,2)
#
(forest_ba_other = (forest_sen_other + forest_spe_other)/2)
round(forest_ba_other,2)

#End: Table 4 random forest confusion matrix####
...
names(r5_imputed.df)
dim(r5_imputed.df)# num payments
# renaming variables for a better visual display
tree_data.df = r5_imputed.df
names(tree_data.df)
tree_data2.df = tree_data.df %>% rename("Preference" = "paypref_inperson", "Merchant" = "merch", "Amount" = "amnt", "Own_credit" = "credit_own", "Credit_reward" = "credit_reward",  "Own_debit" = "debit_own", "Cash_accepted" = "accept_cash", "Card_accepted" = "accept_card", "Discount" = "discount")
names(tree_data2.df)
dim(tree_data2.df)

# Model (recall)
(tree_model1 = pi ~ Preference + Merchant + Amount + Own_credit + Own_debit + Credit_reward + Card_accepted + Cash_accepted +  cc_surcharge + Discount)# without own credit and own debit
...
#
(cp.choice = tree1$cptable[13, "CP"]) # Corresponds to 9 splits (just for demonstration)
...
# Note: Some info in the text are computed at the beginning of Random Forest variable importance table, see around line #808 above
## Card accepted
# raw accept card
...
round(frac_accept_card_imputed,3)# fraction Yes
# by PI
...
## Cash accepted, see also line #834 above
# raw accept card
...
## Credit card surcharge # See also line #864 above
# raw cc_surcharge
table(r4.df$cc_surcharge, useNA = "always")# raw
round(frac_cc_surcharge,3)# fraction Yes # by PI 
...
round(frac_cc_surcharge_imputed,3)# fraction Yes
# by PI
r4_imputed.df %>% group_by(pi) %>% summarise(count =sum(accept_cash =="Yes", na.rm = TRUE))
# by merchant
r4_imputed.df %>% group_by(merch) %>% summarise(count =sum(accept_cash =="Yes", na.rm = TRUE))

## Discount# see also line #878
# raw discount
...
round(frac_discount_imputed,3)# fraction Yes
# by PI
...
# imputed credit reward, see line #974 above.
